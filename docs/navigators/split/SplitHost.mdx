...
Container component that manages a native split-view layout. Renders primary, supplementary, and secondary columns side-by-side on large-screen devices (iPad). Collapses to a single column on compact layouts (iPhone). When collapsed, a specific colum can be shown using [`show`](./TypesReference/SplitHostCommands#show) view command, accessible via [`ref`](#ref) prop.
...
The supported values corresponds to the official UIKit documentation for 
...
See [`DisplayModeWillChangeEvent`](./TypesReference/DisplayModeWillChangeEvent) for full payload type definition.
...
A callback that gets invoked when the [`Split.Inspector`](./SplitInspector) is either programmatically hidden (in column presentation) or dismissed (in modal presentation).
...
UIKit documentation for [`UIInterfaceOrientationMask`](https://developer.apple.com/documentation/uikit/uiinterfaceorientationmask).
...
The supported values corresponds to the official UIKit documentation for [`UISplitViewController.DisplayMode`](https://developer.apple.com/documentation/uikit/uisplitviewcontroller/displaymode-swift.enum).
...
The supported values corresponds to the official UIKit documentation for [`UISplitViewController.SplitBehavior`](https://developer.apple.com/documentation/uikit/uisplitviewcontroller/splitbehavior-swift.enum).
...
The supported values correspond to the official UIKit documentation for [`UISplitViewController.BackgroundStyle`](https://developer.apple.com/documentation/uikit/uisplitviewcontroller/backgroundstyle).
...
The supported values corresponds to the official UIKit documentation for [`UISplitViewController.PrimaryEdge`](https://developer.apple.com/documentation/uikit/uisplitviewcontroller/primaryedge-swift.enum).
...
Reference to the native component. You may use it to call imperative [`SplitHostCommands`](./TypesReference/SplitHostCommands).
