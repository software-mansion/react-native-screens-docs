...
Used by [`Tabs.Host`](../TabsHost#ontabselected).
...
- `programmatic-native` — request initiated from the native side by an actor integrating directly against the native container.
