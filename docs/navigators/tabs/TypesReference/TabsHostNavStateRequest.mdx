...
via [`onTabSelected`](../TabsHost#ontabselected). In other words this should be the provenance
number of last confirmed state you base your update request on.

:::note
This field is named `baseProvenance` (rather than `provenance`) to disambiguate it from the
`provenance` field on the [`TabSelectedEvent`](TabSelectedEvent#provenance) payload, which carries the provenance
...
It must correspond to one of the keys you assign to a `Tabs.Screen`.
